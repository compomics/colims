...
public class MaxQuantProteinGroupParser {
     ...
     * @return a Map key: the protein groupid, value: the parsed proteingroup
     */
    public static Map<Integer, Protein> parseMaxQuantProteinGroups(File aProteinGroupsFile) throws IOException, FileNotFoundException {
        Map<Integer, Protein> proteinGroupMap = new HashMap<>(1000);
        ...
        while (iter.hasNext()) {
            ...
            String test = proteinGroupLine.get(ProteinGroupHeaders.DECOY.headerName);
            boolean isDecoy = test.equals("+");
            //Header header = Header.parseFromFASTA(proteinGroupLine.get(ProteinGroupHeaders.FASTAHEADER.headerName));
            for (String anAccession : proteinGroupLine.get(ProteinGroupHeaders.ACCESSION.headerName).split(",")) {
                proteinGroupMap.put(Integer.parseInt(proteinGroupLine.get(ProteinGroupHeaders.ID.headerName)), new Protein(anAccession, isDecoy));
            }
        }
     ...
     * @return a Map key: the protein group id, value the line number of the protein group
     */
